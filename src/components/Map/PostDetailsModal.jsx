...
export const PostDetailsModal = ({marker, onClose, setIsModalOpen}) => {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center z-10000 bg-opacity-50">
            <div className="flex w-4/12 flex-col bg-main-card">            
              <button className="flex justify-end text-color-primary cursor-pointer pt-1 pr-2"
              onClick={onClose}>X</button>
              <div className="pt-5"></div>
                <section className=" flex justify-between">
                        <h1 className="text-color-primary">{user.fullName}</h1>
                        <h2 className="text-color-primary">{marker.title}</h2>          
                </section>
                    <p className="items-center text-color-primary p-15">{marker.description}</p>
                        {currentUser.id !== marker.userId
                            ? <footer className="flex justify-between">
                                <p className="text-color-primary">{marker.date}</p> 
                            </footer>
